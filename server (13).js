...
app.use((req, res, next) => {
  ...
  express.static(require('path').join(__dirname))(req, res, next);
});
...
app.get('/api/meta/live', auth, async (req, res) => {
  ...
  try {
    const hoy = req.query.hasta || new Date().toISOString().split('T')[0];
    const hace30 = req.query.desde || new Date(Date.now() - 30*24*60*60*1000).toISOString().split('T')[0];

    // Insights por anuncio
    const timeRange = encodeURIComponent(JSON.stringify({since: hace30, until: hoy}));
    const insightsUrl = `https://graph.facebook.com/v19.0/${actId}/ads?fields=name,status,effective_status,created_time,adset{daily_budget,lifetime_budget},insights{spend,impressions,clicks,ctr,cpc,cpm,actions,action_values,website_purchase_roas,date_start,date_stop}&time_range=${timeRange}&limit=100&access_token=${token}`;
    console.log('Meta API desde:', hace30, 'hasta:', hoy);
...
// Wildcard - debe ir AL FINAL después de todas las rutas API
app.get('*', (req, res) => {
