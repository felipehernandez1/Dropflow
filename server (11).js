...
app.get('/api/meta/live', auth, async (req, res) => {
  ...
  try {
    ...
    const insightsUrl = `https://graph.facebook.com/v19.0/${actId}/ads?fields=name,status,effective_status,insights{spend,impressions,clicks,ctr,cpc,cpm,actions,action_values,website_purchase_roas}&time_range={"since":"${hace30}","until":"${hoy}"}&limit=50&access_token=${token}`;
