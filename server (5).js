...
app.get('/api/dropi/pedidos', auth, async (req, res) => {
  ...
  try {
    ...
    const endpointsCandidatos = [
      `/api/v3/orders?page=1&perpage=250&dateFrom=${desde}&dateTo=${hasta}`,
      `/api/v3/orders?page=1&perpage=250`,
      `/api/v2/orders?page=1&perpage=250&dateFrom=${desde}&dateTo=${hasta}`,
      `/api/v2/orders?page=1&perpage=250`,
      `/api/orders?page=1&perpage=250&dateFrom=${desde}&dateTo=${hasta}`,
      `/api/orders?page=1&limit=250`,
      `/api/v1/dropshipper/orders?page=1&per_page=250`,
      `/api/dropshipper/orders?page=1&per_page=250`,
    ];
    ...
    for (const path of endpointsCandidatos) {
      try {
        const result = await fetchDropi(path, row.token);
    ...
    const orders = Array.isArray(dropiData) ? dropiData 
                 : dropiData.data || dropiData.orders || dropiData.result || dropiData.items || [];
...
function fetchDropi(path, token) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'app.dropi.cl',
      path,
      method: 'GET',
      headers: {
