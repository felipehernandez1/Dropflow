...
// ── Planes ──────────────────────────────────────────────────────────
const PLANS = {
  free:        { price:0,     days:null, name:'Free',       features:{ maxOrders: 15,  ai:false, metaAds:false, export:false } },
  pro_weekly:  { price:5990,  days:14,    name:'Pro Weekly', currency:'CLP', features:{ maxOrders:-1, ai:true, metaAds:true, export:true } },
  pro_monthly: { price:9990,  days:30,   name:'Pro Monthly',currency:'CLP', features:{ maxOrders:-1, ai:true, metaAds:true, export:true } },
  pro_usd:     { price:5,     days:14,   name:'Pro Monthly',currency:'USD', features:{ maxOrders:-1, ai:true, metaAds:true, export:true } },
...
app.get('/api/stats', requireAuth, requireAdmin, async (req, res) => {
  ...
  const freeUsers   = await Subs().countDocuments({ plan: 'free' });
  const proUsers    = await Subs().countDocuments({ plan: { $in: ['pro_weekly','pro_monthly','pro_usd','biz_monthly','biz_usd'] }, active: true });
  const today       = new Date(); today.setHours(0,0,0,0);
  const newToday    = await Users().countDocuments({ createdAt: { $gte: today } });
  const thisMonth   = new Date(); thisMonth.setDate(1); thisMonth.setHours(0,0,0,0);
  const newMonth    = await Users().countDocuments({ createdAt: { $gte: thisMonth } });
  res.json({ totalUsers, freeUsers, proUsers, newToday, newMonth });
});
...
app.post('/api/feedback', requireAuth, async (req, res) => {
  ...
  console.log('💡 FEEDBACK [' + type + '] de ' + user + ': ' + message);

  try {
    // Enviar email via Resend
    await axios.post('https://api.resend.com/emails', {
      from:    'Dropflow <onboarding@resend.dev>',
      to:      ['1.felipe.h@gmail.com'],
      subject: '💡 Feedback Dropflow [' + type + '] de ' + user,
      html:    '<h2>Nueva sugerencia de ' + user + '</h2><p><strong>Tipo:</strong> ' + type + '</p><p><strong>Mensaje:</strong></p><p>' + message + '</p><hr><small>Dropflow App</small>',
    }, {
      headers: {
        'Authorization': 'Bearer re_SDgwTj6k_vCjYEVVaqxTNkM6zH8mjm8o1',
        'Content-Type': 'application/json',
      },
      timeout: 10000,
    });
    console.log('✓ Feedback email enviado de', user);
  } catch(e) {
    console.error('Feedback email error:', e.response?.data || e.message);
  }
