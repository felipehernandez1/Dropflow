...
app.post('/api/dropi/login', auth, async (req, res) => {
  const { email, password } = req.body;
  if (!email || !password) return res.status(400).json({ error: 'Email y contraseña requeridos' });
  try {
    // Intentar login en ambos dominios
    let loginOk = false, loginData = null;
    for (const loginUrl of ['https://api.dropi.cl/api/auth/login', 'https://app.dropi.cl/api/auth/login']) {
      try {
      ...
      body: JSON.stringify({ email, password })
          ...
          if (lr.ok) { loginData = await lr.json(); if (loginData.token) { loginOk=true; break; } }
        } catch(e) { console.log('Login error en', loginUrl, e.message); }
      }
    }
    if (!loginOk || !loginData?.token) throw new Error('No se pudo autenticar en Dropi');
    const data = loginData;
    console.log('Dropi login OK, token type:', data.token_type||'bearer');
    ...
    if (existe) {
      db.prepare('UPDATE dropi_tokens SET token=?, email=?, actualizado_en=CURRENT_TIMESTAMP WHERE usuario_id=?')
        .run(data.token, email, req.usuario.id);
    } else {
      db.prepare('INSERT INTO dropi_tokens (usuario_id, token, email) VALUES (?,?,?)').run(req.usuario.id, data.token, email);
    }
...
app.get('/api/dropi/pedidos', auth, async (req, res) => {
  ...
  try {
    // Endpoints conocidos de Dropi — x-authorization header
    // Token JWT: iss=app.dropi.cl, sub=313971, aud=SHOPIFY
    // URL exacta obtenida del Network de Dropi
    // host: api.dropi.cl, endpoint: /api/orders/myorders/v2
    const userId = 313971; // sub del JWT
    const endpointsCandidatos = [
      `/api/orders/myorders/v2?exportAs=orderByRow&orderBy=id&orderDirection=desc&result_number=250&start=0&textToSearch=&status=null&supplier_id=false&user_id=${userId}&from=${desde}&until=${hasta}&filter_product=undefined&haveIncidenceProcesamiento=false&tag_id=&warranty=false&seller=null&filter_date_by=FECHA%20DE%20CREADO&invoiced=null`,
      `/api/orders/myorders/v2?exportAs=orderByRow&orderBy=id&orderDirection=desc&result_number=250&start=0&user_id=${userId}&from=${desde}&until=${hasta}`,
      `/api/orders/myorders/v2?result_number=250&start=0&from=${desde}&until=${hasta}`,
    ];

    let dropiData = null;
    let endpointUsado = '';

    for (const path of endpointsCandidatos) {
      try {
        const result = await fetchDropi(path, row.token, 'api.dropi.cl');
        // Verificar que devolvió datos válidos (no error ni HTML)
        if (result && (result.data || result.orders || result.result || result.items || Array.isArray(result))) {
          dropiData = result;
          endpointUsado = path;
          console.log('✓ Dropi endpoint OK:', path);
          break;
        } else if (result?.isSuccess === false || result?.error) {
          console.log('✗ Dropi endpoint error:', path, result?.message || result?.error);
        } else {
          console.log('? Dropi endpoint respuesta inesperada:', path, JSON.stringify(result).substring(0,80));
        }
      } catch(e) {
        console.log('✗ Dropi endpoint excepción:', path, e.message.substring(0,80));
      }
    }

    if (!dropiData) {
      throw new Error('No se pudo conectar a Dropi. Revisa que el token sea válido y esté activo.');
    }

    // La respuesta de /api/orders/myorders/v2 viene con estructura específica
    let orders = [];
    if (Array.isArray(dropiData)) {
      orders = dropiData;
    } else if (dropiData.data) {
      orders = Array.isArray(dropiData.data) ? dropiData.data : dropiData.data.data || [];
    } else {
      orders = dropiData.orders || dropiData.result || dropiData.items || [];
    }
    
    console.log(`✓ Dropi: ${orders.length} pedidos via ${endpointUsado}`);
    if (orders.length > 0) console.log('Primer pedido campos:', Object.keys(orders[0]).join(', '));
    ...
    const pedidos = orders.map(o => {
      // Mapeo con campos reales de Dropi API /myorders/v2
      const estadoRaw = o.status || o.ESTATUS || o.estado || '';
      const venta = parseFloat(o.sale_price || o.VALOR_DE_COMPRA_EN_PRODUCTOS || o.valor_venta || o.total || 0);
      const proveedor = parseFloat(o.provider_price || o.TOTAL_EN_PRECIOS_DE_PROVEEDOR || o.valor_proveedor || o.product_cost || 0);
      const flete = parseFloat(o.shipping_cost || o.PRECIO_FLETE || o.precio_flete || o.freight || 0);
      const comision = parseFloat(o.commission || o.COMISION || o.comision || 0);
      const devFlete = parseFloat(o.return_cost || o.COSTO_DEVOLUCION_FLETE || 0);
      const utilidad = parseFloat(o.utility || o.GANANCIA || o.ganancia || (venta - proveedor - flete - comision - devFlete));
      return {
        id:        String(o.id || o.order_id || ''),
        fecha:     (o.created_at || o.FECHA || o.fecha || '').split('T')[0] || '',
        cliente:   o.customer_name || o.NOMBRE_CLIENTE || o.cliente || '',
        ciudad:    o.city || o.CIUDAD_DESTINO || o.ciudad_destino?.nombre || '',
        producto:  o.product_name || o.CATEGORIAS || o.producto || 'Sin nombre',
        estadoRaw,
        estado:    mapEstadoDropi(estadoRaw),
...
function fetchDropi(path, token, host='api.dropi.cl') {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: host,
      path,
      ...
      headers: {
        ...
        'Referer': 'https://app.dropi.cl/dashboard/orders',
      }
    };
    console.log('Dropi GET:', options.hostname + path);
    const req = https.request(options, (apiRes) => {
      let data = '';
      console.log('Dropi status:', apiRes.statusCode);
      apiRes.on('data', chunk => data += chunk);
      apiRes.on('end', () => {
        console.log('Dropi response:', data.substring(0, 300));
        try {
          const json = JSON.parse(data);
          resolve(json);
        } catch(e) {
          reject(new Error('Dropi no devolvió JSON: ' + data.substring(0, 100)));
        }
      });
    });
    req.on('error', reject);
